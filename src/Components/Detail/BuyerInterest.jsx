...
const App = () => {
  ...
  const fetchInterestedProperties = async () => {
    try {
      ...
      if (response.status === 200) {
        const transformedProperties = response.data.propertiesData.flatMap(property =>
          property.interestedUsers.map(interestedUser => ({
            ...property,
            interestedUser
          }))
        );
        ...
        // toast.error('No properties found for this owner.');
      }
    } catch (error) {
  ...
  const handleUndoRemove = async (ppcId, interestedUser) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/undo-delete-property`, { ppcId, phoneNumber: interestedUser });
