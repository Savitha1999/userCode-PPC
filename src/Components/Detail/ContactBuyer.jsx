...
const App = () => {
  ...
  useEffect(() => {
    ...
    const fetchInterestedProperties = async () => {
      try {
        const apiUrl = `${process.env.REACT_APP_API_URL}/get-contact-buyer`;  // Buyer API endpoint
        const response = await axios.get(apiUrl, {
          params: { postedPhoneNumber: phoneNumber }  // Send phoneNumber as a query parameter
        });

        // if (response.status === 200) {
        //   setProperties(response.data.contactRequestsData || []);  // Set properties data
        // } 
        if (response.status === 200) {
          const transformedProperties = response.data.contactRequestsData.flatMap(property =>
            property.contactRequestersPhoneNumbers.map(interestedUser => ({
              ...property,
              interestedUser
            })) 
          );
