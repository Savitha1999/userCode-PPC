...
const App = () => {
  ...
  useEffect(() => {
    ...
    const fetchSoldoutProperties = async () => {
      try {
        const apiUrl = `${process.env.REACT_APP_API_URL}/get-soldout-buyer`;  // Buyer API endpoint
        ...
        if (response.status === 200) {
          const transformedProperties = response.data.soldOutRequestsData.flatMap(property =>
            property.soldOutRequestersPhoneNumbers.map(interestedUser => ({
              ...property,
              interestedUser
            }))
          );
